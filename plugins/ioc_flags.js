...
function tweReplaceEmoji(el) {
	...
	var category = {
		...
		'AsahiMitsuyaJapan_2020': [
			['3月28日は三ツ矢の日', '三ツ矢サイダー', '嵐と三ツ矢でカンパイ', '日本にはおいしいサイダーがある']
		],
		...
		'WHO_SafeHands_2020': [
			['手洗い', '手洗いチャレンジ', 'HANDWASHING', 'SAFEHANDS', 'HANDWASHCHALLENGE', 'HANDWASHINGCHALLENGE']
		],
		...
		'MentalHealthMonth_2021_add2': [
			['一人で抱え込まないで', 'RECOVERTOGETHER', 'TOGETHERWECAN', 'WECARE', '話すのも悪くない', 'LETSTALK']
		],
		...
		'HumanRightsDay_2020': [
			['世界人権デー', '人権デー', '人権', 'HUMANRIGHTSDAY', 'HUMANRIGHTS', 'STANDUP4HUMANRIGHTS']
		],
		'SpaceSunrise_2020': [
	...
	Object.keys(category).forEach(function(key) {
		category[key].forEach(function(names) {
			names.forEach(function(name) {
				organization[name.toUpperCase()] = key.replace('#', names[0]);
			});
		});
	});
